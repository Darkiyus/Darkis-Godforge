...
describe("API data workflows", () => { it("exports and imports deity data and caches catalog reads", async () => { const service = new DeityService(); service.save(deity); const api = new GodForgeApi(service, new AdapterRegistry()); const first = await api.getSelectableDeities({}); expect(await api.getSelectableDeities({})).toBe(first); const exported = api.exportDeities("2026-07-20T00:00:00.000Z"); expect(api.importDeities(exported)).toBe(1); expect(api.drawRandomDeity(() => 0).entry.id).toBe("a"); }); });
