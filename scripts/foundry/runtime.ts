...
export interface FoundryRuntime { Hooks: FoundryHooks; game?: { user?: { id?: string; isGM?: boolean }; system?: { id?: string }; journal?: FoundryJournalCollection; settings?: { register(namespace: string, key: string, config: Record<string, unknown>): void; get?(namespace: string, key: string): unknown }; keybindings?: { register(namespace: string, key: string, config: Record<string, unknown>): void }; modules?: { get(id: string): { api?: unknown; languages?: Array<{ lang: string; name: string }> } | undefined } }; }
