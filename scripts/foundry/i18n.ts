...
export function localize(key: string): string { const runtime = globalThis as unknown as { game?: { settings?: { get?: (namespace: string, key: string) => unknown }; i18n?: { localize?: (key: string) => string } } }; const language = runtime.game?.settings?.get?.("darkis-godforge", "language"); if (typeof language === "string" && language !== "auto") { const value = lookup(catalogs.get(language), key); if (typeof value === "string") return value; } const foundryValue = runtime.game?.i18n?.localize?.(key); if (foundryValue && foundryValue !== key) return foundryValue; const fallback = lookup(english as Catalog, key); return typeof fallback === "string" ? fallback : key; }
