...
export class GodForgeDeityDetail extends gmApplicationBase() {
  ...
  async _prepareContext(): Promise<Record<string, unknown>> { requireGM(); return { ui: uiText(), deity: { ...this.deity, image: safeImageUrl(this.deity.image), ...imagePresentationStyle(this.deity.imagePresentation?.image) } }; }
