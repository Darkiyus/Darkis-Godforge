...
export function validateExport(value: unknown): value is GodForgeExport { if (!value || typeof value !== "object") return false; const candidate = value as Partial<GodForgeExport>; return candidate.format === "darkis-godforge" && candidate.schemaVersion === 1 && Array.isArray(candidate.deities) && candidate.deities.every((deity) => typeof deity === "object" && deity !== null && typeof deity.id === "string" && typeof deity.name === "string"); }
export function importDefinitions(value: unknown): DeityDefinition[] { if (!validateExport(value)) throw new Error("Invalid GodForge export: expected a version 1 deity export."); return structuredClone(value.deities); }
