...
import { discoveryForViewer, isDeityVisible, redactForViewer, type ViewerContext } from "./visibility-service";
...
export function filterCatalog(deities: DeityDefinition[], context: SelectionContext, hidden: Set<string>, viewer: ViewerContext = { isGM: true }): DeitySummary[] {
  return deities
    .filter((deity) => deity.kind !== "lore" && !hidden.has(deity.id) && isDeityVisible(deity, viewer) && discoveryForViewer(deity, viewer) === "revealed" && (!context.pantheonFilter || deity.domains.includes(context.pantheonFilter)))
    .flatMap((deity) => {
