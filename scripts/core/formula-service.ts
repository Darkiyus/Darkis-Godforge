...
export async function evaluateFormulaWithDice(formula: string, facts: FormulaFacts, rollDice: (term: string) => Promise<number>): Promise<number> { if (!validateFormula(formula)) throw new Error("Formula contains an unsupported term."); const terms = formula.replace(/\s/g, "").match(/\b\d+d\d+\b/g) ?? []; let resolved = formula; for (const term of [...new Set(terms)]) { const result = await rollDice(term); if (!Number.isFinite(result)) throw new Error("Dice result is not a finite number."); resolved = resolved.replace(new RegExp(`\\b${term}\\b`, "g"), String(result)); } return evaluateFormula(resolved, facts); }
