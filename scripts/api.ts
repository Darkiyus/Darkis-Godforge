...
export class GodForgeApi {
  ...
  drawRandomDeity(random: () => number): RandomDraw { const viewer = currentViewerContext(true); return drawWeighted(this.deities.list().filter((deity) => deity.kind !== "lore" && isDeityVisible(deity, viewer)).map((deity) => ({ id: deity.id, label: deity.name, weight: 1 })), random); }
  getAdapterCapabilities(systemId: string) { return this.adapters.get(systemId).capabilities; }
  isDeitySelectableByPlayer(deityId: string): boolean { const deity = this.deities.get(deityId); return Boolean(deity && deity.kind !== "lore" && isDeityVisible(deity, { isGM: false, selection: true })); }
